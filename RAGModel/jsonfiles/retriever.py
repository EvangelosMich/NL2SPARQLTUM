...
prop_texts: List[str] = [f"{v} ({k})" for k, v in properties.items()]
...
def retrieve_offline_ids(
    ...
    *,
    ...
    # Property search
    _, prop_idx = prop_index.search(query_emb, topk_prop)
    prop_hits = [prop_ids[i] for i in prop_idx[0]]
